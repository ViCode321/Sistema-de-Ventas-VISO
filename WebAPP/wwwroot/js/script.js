...
$(document).ready(function () {
    ...
    $('.open-layout').on("click", function (s) {
        s.preventDefault(); $('.sidebar-layout').addClass('show-layout'); $('.sidebar-settings').removeClass('show-settings');
    });
    $('.btn-closed').on("click", function (s) {
        s.preventDefault(); $('.sidebar-layout').removeClass('show-layout');
    }); $('.open-settings').on("click", function (s) {
        s.preventDefault(); $('.sidebar-settings').addClass('show-settings'); $('.sidebar-layout').removeClass('show-layout');
    }); $('.btn-closed').on("click", function (s) {
        s.preventDefault(); $('.sidebar-settings').removeClass('show-settings');
    }); $('.open-siderbar').on("click", function (s) {
        s.preventDefault(); $('.siderbar-view').addClass('show-sidebar');
    }); $('.btn-closed').on("click", function (s) {
        s.preventDefault();
        $('.siderbar-view').removeClass('show-sidebar');
    });

    if ($('.toggle-switch').length > 0 || $('#theme-switch-dropdown').length > 0) {
        const toggleSwitch = document.querySelector('.toggle-switch input[type="checkbox"]');
        const dropdownToggleSwitch = document.getElementById('theme-switch-dropdown');
        ...
        if (currentTheme) {
            ...
            if (currentTheme === 'dark') {
                ...
                if (dropdownToggleSwitch) dropdownToggleSwitch.checked = true;
            }
        }

        function switchTheme(e) {
            var logo = document.querySelector('.logo img');
            var logo_small = document.querySelector('.logo-small img');
            if (e.target.checked) {
                app.setAttribute('data-theme', 'dark');
                localStorage.setItem('theme', 'dark');
                logo.src = '/img/icons/viso2.jpeg';
                logo_small.src = '/img/icons/viso2_2.jpeg';
            } else {
                app.setAttribute('data-theme', 'light');
                localStorage.setItem('theme', 'light');
                logo.src = '/img/icons/viso1.jpeg';
                logo_small.src = '/img/icons/viso1_1.jpeg';
            }
        }
        ...
        if (toggleSwitch) {
            toggleSwitch.addEventListener('change', function (e) {
                switchTheme(e);
                dropdownToggleSwitch.checked = e.target.checked;
            }, false);
        }

        if (dropdownToggleSwitch) {
            document.getElementById('theme-toggle').addEventListener('click', function (e) {
                dropdownToggleSwitch.checked = !dropdownToggleSwitch.checked;
                switchTheme({ target: dropdownToggleSwitch });
                if (toggleSwitch) toggleSwitch.checked = dropdownToggleSwitch.checked;
            }, false);
        }
    }
